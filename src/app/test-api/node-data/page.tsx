...
import { NodeDataContext, NavDataContext } from "@/redux/provider";
...
export default function Page() {
  ...
  if (!nodeData) {
    return (
      <div className="p-4">
        <h1 className="text-xl font-bold mb-4">No node data available</h1>
      </div>
    );
  }
        ...
        <div className="bg-gray-100 p-4 rounded overflow-auto max-h-[400px]">
          {nodeData.links.map((link, index) => (
            <div key={link.relationship_id} className="mb-2 p-2 border-b">
              ...
              <p>
                {nodeMap[link.source]} ({link.source}) → {nodeMap[link.target]}{" "}
                ({link.target})
              </p>
            </div>
          ))}
